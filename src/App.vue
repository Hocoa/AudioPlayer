...
<script>
  ...
  let linesGroup, outLine, inLine, barLine = [], barNodes;// 线
  let barGroup; // 柱子
  let Triangles=[], TriangleGroup;
    ...
    methods: {
      ...
      updateCircle(arr) {
        ...
        if (barNodes) {
          ...
          const AttributeA = geometryA.getAttribute(
                  "position"
          );
          const geometryB = inLine.geometry;
          const AttributeB = geometryB.getAttribute(
                  "position"
          );
      ...
      audioLines(radius, countData) {
        ...
        const lineMaterial = new THREE.LineBasicMaterial({color: gui.lineColor});
        ...
        outLine = new THREE.Line(
                new THREE.BufferGeometry().setAttribute(
                        "position",
                        this.renderGeometries(
                                barNodes.map((node) => node.positionA())
                        )
                ),
                lineMaterial
        );
        ...
        inLine = new THREE.Line(
                new THREE.BufferGeometry().setAttribute(
                        "position",
                        this.renderGeometries(
                                barNodes.map((node) => node.positionB())
                        )
                ),
                lineMaterial
        );
      ...
      addTriangle() {
        const material = new THREE.MeshBasicMaterial({ color: gui.TrianglesBgColor });
        const lineMaterial = new THREE.LineBasicMaterial({ color: gui.TrianglesLineColor });
      ...
      initGui() {
        ...
        datGui.addColor(gui, 'TrianglesBgColor')
                .onChange(function () {
                  TriangleGroup.traverse(function (child) {
                    if (child.isMesh) child.material = new THREE.MeshPhongMaterial({ color: gui.TrianglesBgColor })
                  });
                });
        datGui.addColor(gui, 'TrianglesLineColor')
                .onChange(function () {
                  TriangleGroup.traverse(function (child) {
                    if (child.isLine) child.material = new THREE.LineBasicMaterial({ color: gui.TrianglesLineColor })
                  });
                });
        datGui.addColor(gui, 'lineColor')
                .onChange(function () {
